...
func TestE文本编码转换utf8(t *testing.T) {
	...
	E调试输出("isUtf8:", E编码是否为utf8([]byte(str)))         //判断是否是utf-8
	gbkData := E编码utf8到gbk(str)                       //使用官方库将utf-8转换为gbk
	E调试输出("gbk直接打印会出现乱码:", gbkData)                   //乱码字符串
	E调试输出("isGBK:", E编码是否为gbk([]byte(gbkData)))       //判断是否是gbk
	utf8Data := E编码gbk到utf8(gbkData)                  //将gbk再转换为utf-8
	E调试输出("isUtf8:", E编码是否为utf8([]byte(utf8Data)))    //判断是否是utf-8
...
func TestBase64解码(t *testing.T) {
	base64string := X编码Base64(E到字节集("abcdefg"))
	E调试输出(base64string)
	E调试输出(X编码Base64解码(base64string))
	E调试输出(X编码URL("<>?"))
	E调试输出(X编码URL解码("%3C%3E%3F"))
	E调试输出(X编码URL解析("http://user:pass@example.com:8080/path/to/index.html", -1))
