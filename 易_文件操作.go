...
// X读入文件 从指定文件名读取数据并返回读取到的字节切片。
// 如果读取失败，则返回一个空切片和错误信息。
func X读入文件(文件名 string) []byte {
	var data []byte
	data, _ = os.ReadFile(文件名)
	return data
}

// X读入文本 从指定文件名读取数据并返回读取到的文本
// 如果读取失败，则返回一个空切片和错误信息。
func X读入文本(文件名 string) string {
	var data []byte
	data, _ = os.ReadFile(文件名)
	return string(data)
}

// X写到文件 将指定的数据写入指定的文件中。如果写入成功，返回 nil。否则，返回错误信息。
func X写到文件(文件名 string, 欲写入文件的数据 []byte) error {
	父目录 := X文件取父目录(文件名)
	if !X文件是否存在(父目录) {
		X创建目录多级(父目录)
	}
...
func X取当前目录() string {
	...
	if err != nil {
		//提示错误 函数名 文件名 错误信息
		log.Printf("func:%s file:%s err:%s", "X取当前目录", "文件操作.go", err)
		return ""
	}
	return dir
}
...
// X置当前目录 设置当前程序运行的目录。
func X置当前目录(目录 string) error {
	return os.Chdir(目录)
}

// X创建目录 创建一个目录。如果创建成功，则返回 nil。否则，返回错误信息。
func X创建目录(欲创建的目录名称 string) error {
...
func X删除目录(欲删除的目录名称 string) error {
...
// 返回值：
// error - 操作失败会返回一个非-nil的错误对象
func X复制文件(被复制的文件名 string, 复制到的文件名 string) error {
...
// 返回错误信息 error 如果发生错误会返回错误信息 成功返回nil
func X删除文件(欲删除的文件名 string) error {
...
// 返回错误信息 error 如果发生错误会返回错误信息 成功返回nil
func X文件更名(欲更名的原文件或目录名 string, 欲更改为的现文件或目录名 string) error {
...
func X文件是否存在(欲测试的文件名称 string) bool {
...
// X取临时文件名 在指定目录中创建一个临时文件并返回其文件对象、完整路径及错误信息
// 目录名 string 要在哪个目录下创建临时文件
// 返回 *os.File 临时文件的文件对象
// 返回 filepath 临时文件的完整路径
// 返回 error 错误信息 成功时为nil
func X取临时文件名(目录名 string) (f *os.File, filepath string, err error) {
	prefix := ""
...
func X复制目录(被复制的目录名称 string, 复制到的目录名称 string) error {
	// 创建目标目录
...
func X文件枚举(欲寻找的目录 string, 欲寻找的文件名 string, files *[]string, 是否带路径 bool, 是否遍历子目录 bool) error {
	var ok bool
	...
	for _, f := range l {
		...
		if f.IsDir() {
			if 是否遍历子目录 {
				err = X文件枚举(tmp, 欲寻找的文件名, files, 是否带路径, 是否遍历子目录)
				if err != nil {
					return err
				}
			}
...
func X目录枚举子目录(父文件夹路径 string, 子目录数组 *[]string, 是否带路径 bool, 是否继续向下枚举 bool) error {
	...
	for _, f := range l {
		...
		if f.IsDir() {
			...
			if 是否继续向下枚举 {
				err = X目录枚举子目录(tmp, 子目录数组, 是否带路径, 是否继续向下枚举)
				if err != nil {
					return err
				}
			}
...
func X文件取文件名(路径 string, 是否需要后缀 bool) string {
	if 是否需要后缀 {
		return filepath.Base(路径)
	}
	return strings.TrimSuffix(filepath.Base(路径), filepath.Ext(路径))
}
func X文件路径合并处理(elem ...string) string {
	return path.Join(elem...)
}

func X文件取父目录(dirpath string) string {
	return path.Dir(dirpath)
}

func X文件取扩展名(filepath string) string {
	return path.Ext(filepath)
}

func X文件删除(欲删除的文件名 string) bool {
	return X删除文件(欲删除的文件名) == nil
}

// 路径不存在时自动创建
func X文件写出(文件名 string, 欲写入文件的数据 interface{}) error {
	fpath := X文件取父目录(文件名)
	if !X文件是否存在(fpath) {
		X创建目录多级(fpath)
	}
	return ioutil.WriteFile(文件名, X到字节集(欲写入文件的数据), os.ModePerm)
}

// 路径不存在时自动创建
func X文件追加文本(文件名 string, 欲追加的文本 string) error {
	fpath := X文件取父目录(文件名)
	if !X文件是否存在(fpath) {
		X创建目录多级(fpath)
	}
	...
	_, err = file.Write(X到字节集(欲追加的文本 + "\r\n"))
	return err
}

// 自动检查内容是否一致是否需要写出
func X文件保存(文件名 string, 欲写入文件的数据 interface{}) error {
	if X文件是否存在(文件名) {
		data := X读入文件(文件名)
		wdata := X到字节集(欲写入文件的数据)
		if !bytes.Equal(data, wdata) {
			//X调试输出("不相同写出")
			return X文件写出(文件名, wdata)
		}
		//X调试输出("内容一样不写出")
	} else {
		//X调试输出("文件不存在写出")
		return X文件写出(文件名, 欲写入文件的数据)
	}
	return nil
}

// X取文件Mime
func X取文件Mime(文件路径 string) string {
