...
// X取文本长度 根据UTF-8编码计算字符串长度
func X取文本长度(value string) int64 {
	return X到整数(len([]rune(value)))
}

// X取文本左边 从文本的左边取出指定数目的字符
func X取文本左边(欲取其部分的文本 string, 欲取出字符的数目 int64) string {
	if X取文本长度(欲取其部分的文本) < 欲取出字符的数目 {
		欲取出字符的数目 = X取文本长度(欲取其部分的文本)
	}
	return string([]rune(欲取其部分的文本)[:欲取出字符的数目])
}

// X取文本右边 从文本的右边取出指定数目的字符
func X取文本右边(欲取其部分的文本 string, 欲取出字符的数目 int64) string {
	l := X取文本长度(欲取其部分的文本)
	lpos := l - 欲取出字符的数目
	if lpos < 0 {
		lpos = 0
...
// X取文本中间 从文本的中间取出指定数目的字符
func X取文本中间(欲取其部分的文本 string, 起始位置 int64, 欲取出字符的数目 int64) string {
	if 起始位置 < 0 {
		起始位置 = 0
	}
	if 欲取出字符的数目 < 0 {
...
func X字符(欲取其字符的字符代码 int8) string {
...
func X取代码(欲取字符代码的文本 string) int {
...
func X寻找文本(被搜寻的文本 string, 欲寻找的文本 string, 起始搜寻位置 int, 是否不区分大小写 bool) int {
	if 起始搜寻位置 < 1 {
		起始搜寻位置 = 1
	}
...
// 参数<4>的名称为“是否不区分大小写”，类型为“逻辑型（bool）”，初始值为“假”。为真不区分大小写，为假区分。
func X倒找文本(被搜寻的文本 string, 欲寻找的文本 string, 起始搜寻位置 int, 是否不区分大小写 bool) int {
	r1 := []rune(被搜寻的文本)
	r2 := []rune(欲寻找的文本)
	r1Len := len(r1)
	r2Len := len(r2)
...
// X到大写 转换为大写
func X到大写(value string) string {
	return strings.ToUpper(value)
}

// X到小写 转换为小写
func X到小写(value string) string {
	return strings.ToLower(value)
}

// X到全角 转换为全角
func X到全角(value string) string {
	return sBCtoDBC(value)
}

// X到半角 转换为半角
func X到半角(value string) string {
...
func X删首空(欲删除空格的文本 string) string {
...
func X删尾空(欲删除空格的文本 string) string {
	return strings.TrimRight(欲删除空格的文本, " ")
}

func X删首尾空(内容 string) string {
	return strings.TrimSpace(内容)
}

// 删全部空
func X删全部空(内容 string) string {
...
func X文本替换(欲被替换的文本 string, 起始替换位置, 替换长度 int, 用作替换的文本 string) string {
	r := []rune(欲被替换的文本)
	rLen := len(r)
...
func X子文本替换(欲被替换的文本 string, 欲被替换的子文本 string, 用作替换的子文本 string, 进行替换的起始位置 int, 替换进行的次数 int, 是否区分大小写 bool) string {
	if 替换进行的次数 == 0 {
		return 欲被替换的文本
	}
	if len(用作替换的子文本) == 0 {
...
func X取重复文本(重复次数 int, 待重复文本 string) string {
	if 待重复文本 == "" {
		return ""
	}
...
func X分割文本(待分割文本 string, 用作分割的文本 string) []string {
...
func StrCut(内容 string, 表达式 string) string {
	...
	if len(args) == 2 {
		return X文本取出中间文本(内容, args[0], args[1])
	}
	if len(args) == 1 {
		return X文本取出中间文本(内容, args[0], "")
	}
	return ""
}

func X文本取左边(被查找的文本 string, 欲寻找的文本 string) string {
	return X文本取出中间文本(被查找的文本, "", 欲寻找的文本)
}
func X文本取右边(被查找的文本 string, 欲寻找的文本 string) string {
	return X文本取出中间文本(被查找的文本, 欲寻找的文本, "")
}

// 文本取出中间文本
func X文本取出中间文本(内容 string, 左边文本 string, 右边文本 string) string {
...
func X文本取随机字母(要取的字符数 int, 字母类型 int) string {
	var str string
	if 字母类型 == 0 {
...
func X文本取随机字母和数字(要取的字符数 int) string {
	return grand.Str("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ", 要取的字符数)
}

func X文本取随机数字(要取的字符数 int) string {
	return grand.Str("0123456789", 要取的字符数)
}

func X文本删左边(欲处理文本 string, 删除长度 int64) string {
	return X取文本右边(欲处理文本, X取文本长度(欲处理文本)-删除长度)
}
func X文本删右边(欲处理文本 string, 删除长度 int64) string {
	return X取文本左边(欲处理文本, X取文本长度(欲处理文本)-删除长度)
}

func X文本删中间(欲处理文本 string, 起始位置 int64, 删除长度 int64) string {
	return X取文本左边(欲处理文本, 起始位置) + X文本删左边(欲处理文本, 起始位置+删除长度)
}

func X文本取出文本中汉字(s string) string {
	return X文本区分_只取汉子(s)
}

func X文本逐字分割(s string) []string {
	r := []rune(s)
	strarr := []string{}
	for _, s := range r {
...
func X文本自动补零(s string, len int) string {
	return X格式化文本("%0*d", len, X到整数(s))
}
...
func X文本区分_只取字母(s string) string {
...
func X文本区分_只取数字(s string) string {
	str := ""
	for _, r := range s {
		if unicode.IsNumber(r) {
			str = str + string(r)
...
func X文本区分_只取汉子(s string) string {
	str := ""
	for _, r := range s {
		if unicode.Is(unicode.Scripts["Han"], r) {
			str = str + string(r)
...
func X文本区分_只取符号(s string) string {
	str := ""
	for _, r := range s {
...
func X文本首字母改大写(s string) string {
	if len(s) < 1 {
		return ""
	}
	strArry := []rune(s)
...
func X判断文本(内容 string, 关键字 ...interface{}) bool {
	for _, v := range 关键字 {
		str := X到文本(v)
...
func X判断文本s(内容 string, 关键字 ...interface{}) string {
	for _, v := range 关键字 {
		str := X到文本(v)
		if strings.Index(内容, str) != -1 {
			return str
...
func X判断文本前缀(s string, 前缀 string) bool {
	return strings.HasPrefix(s, 前缀)
}
func X判断文本后缀(s string, 后缀 string) bool {
	return strings.HasSuffix(s, 后缀)
}

func X文本单词首字母大写(s string) string {
	return gstr.UcFirst(s)
}

func X文本句子首字母大写(s string) string {
	return gstr.UcWords(s)
}

func X文本自动换行(s string, 每行几个字符 int, 分隔符 string) string {
	return gstr.WordWrap(s, 每行几个字符, 分隔符)
}

// SimilarText计算两个字符串之间的相似度。参见http://php.net/manual/en/function.similar-text.php。
func X文本相似文本(first, second string, percent *float64) int {
	return gstr.SimilarText(first, second, percent)
}

func X文本随机文本(s string) string {
	return gstr.Shuffle(s)
}

func X文本搜索切片文本(a []string, s string) int {
