...
func (a *StrArray) X加入成员(s string) {
	a.X父类.Append(s)
}
func (a *StrArray) X取数组成员数() int {
	return a.X父类.Len()
}

func (a *StrArray) X删除成员(欲删除的位置 int) {
	a.X父类.Remove(欲删除的位置)
}

func (a *StrArray) X清除数组() {
	a.X父类.Clear()
}
...
func (a *StrArray) X数组排序(排序方向是否为从小到大 bool) {
...
func (a *StrArray) X插入成员(欲插入的位置 int, 欲插入的成员数据 string) {
	a.X父类.InsertBefore(欲插入的位置, 欲插入的成员数据)
}
func (a *StrArray) X取值(i int) string {
	返回值, _ := a.X父类.Get(i)
	return 返回值
}
